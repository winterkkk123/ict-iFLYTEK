<template>
  <div class="question">
    <input type="text" :value="inputValue" @input="updateInput" @keydown.enter="emitSearchClick" placeholder="在此输入内容..." class="input">
...
<script lang="ts" setup name="Question">
  ...
  const emitSearchClick = async () => {
    ...
    try {
      // 发送请求到服务器
      const response = await fetch('http://118.178.138.32:8081/agent/question/itQuery', {
        method: 'POST',
        headers: {
